...
public class EditCommand extends Command {
    ...
    private void parseParameters(AbsSender sender, Update update, Long chatId) {
        ...
        switch (documentCursor.get(chatId).getType()) {
            case "book":
    ...
    private void selectDocToEdit(AbsSender sender, Update update, Long chatId, String type) {
        ...
        switch (type) {
            case "book":
                item = SqlStorage.getInstance().findBooks(new QueryParameters()).get(index - 1);
                break;
            case "avmaterial":
                item = SqlStorage.getInstance().findAvMaterials(new QueryParameters()).get(index - 1);
                break;
            case "journalissue":
                item = SqlStorage.getInstance().findJournals(new QueryParameters()).get(index - 1);
                break;
            case "journalarticle":
                item = SqlStorage.getInstance().findArticles(new QueryParameters()).get(index - 1);
                break;
        }

        if (item != null) {
            documentCursor.put(chatId, item);
            sendMessage(sender, update, item.toString());
